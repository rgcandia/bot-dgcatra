...
export default function BasesPage() {
  ...
  async function load() {
    try {
      const data = await api.get<Base[]>('/api/bases');
      setBases(data);
  ...
  async function handleSave() {
    ...
    try {
      if (edit.id) {
        await api.patch(`/api/bases/${edit.id}`, { nombre: edit.nombre, codigoAcceso: edit.codigoAcceso });
      } else {
        await api.post('/api/bases', { nombre: edit.nombre, codigoAcceso: edit.codigoAcceso });
      }
  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem' }}>
        <h2>Bases</h2>
        <button className="btn btn-primary" onClick={() => { setEdit({ nombre: '', codigoAcceso: '' }); setShowNew(true); }}>
          Nueva base
        </button>
      </div>
        ...
        <tbody>
          {bases.map(b => (
            <tr key={b.id}>
              ...
              <td>
                <button className="btn btn-ghost btn-sm" onClick={() => setEdit(b)}>Editar</button>
                <button className="btn btn-ghost btn-sm" style={{ color: '#dc2626' }} onClick={() => handleDelete(b.id)}>Borrar</button>
              </td>
            </tr>
          ))}
        </tbody>
